...
class WordCounter(Process):
    ...
    def __init__(self):
        inputs = [
            ComplexInput(
                "text",
                "Text document",
                abstract='URL pointing to a text document, for example "Alice in Wonderland": http://www.gutenberg.org/cache/epub/19033/pg19033.txt',
                supported_formats=[FORMATS.TEXT],
            ),
        ]
        outputs = [
            ComplexOutput("output", "Word counter result", as_reference=True, supported_formats=[FORMATS.JSON]),
        ]
    ...
    def _handler(self, request, response):
        ...
        def words(f):
            for line in f:
                yield from wordre.findall(line.decode("UTF-8"))
