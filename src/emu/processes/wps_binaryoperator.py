...
class BinaryOperator(Process):
    def __init__(self):
        inputs = [
            LiteralInput("inputa", "Input 1", data_type="float", abstract="Enter Input 1", default="2.0"),
            LiteralInput("inputb", "Input 2", data_type="float", abstract="Enter Input 2", default="3.0"),
            LiteralInput(
                "operator",
                "Operator",
                data_type="string",
                ...
                default="add",
                allowed_values=["add", "subtract", "divide", "multiply"],
            ),
        ]
        outputs = [LiteralOutput("output", "Binary operator result", data_type="float")]

        super().__init__(
            self._handler,
            identifier="binaryoperatorfornumbers",
            title="Binary Operator for Numbers",
            abstract="Performs operation on two numbers and returns the answer. This example process is taken from Climate4Impact.",
            metadata=[
