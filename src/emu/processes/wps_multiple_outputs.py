...
class MultipleOutputs(Process):
    def __init__(self):
        ...
        outputs = [
            ComplexOutput(
                "output", "METALINK v3 output", abstract="Testing metalink v3 output", as_reference=True, supported_formats=[FORMATS.METALINK]
            ),
            ComplexOutput(
                "output_meta4", "METALINK v4 output", abstract="Testing metalink v4 output", as_reference=True, supported_formats=[FORMATS.META4]
            ),
        ]
        ...
        super().__init__(
            ...
            abstract="Produces multiple files and returns a document with references to these files.",
            metadata=[
    ...
    def _handler(self, request, response):
        ...
        for i in range(max_outputs):
            mf = MetaFile(f"output_{i}", "Test output", fmt=FORMATS.TEXT)
            mf.data = f"output: {i}"
            ml3.append(mf)
