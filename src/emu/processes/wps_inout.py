...
class InOut(Process):
    ...
    def __init__(self):
        inputs = [
            LiteralInput(
                "string", "String", data_type="string", abstract="Enter a simple string.", default="This is just a string", mode=MODE.SIMPLE
            ),
            LiteralInput(
                "int",
                "Integer",
            ...
            LiteralInput("float", "Float", data_type="float", abstract="Enter a float number.", default="3.14", min_occurs=0, max_occurs=5),
            # TODO: boolean default is not displayed in phoenix
            LiteralInput("boolean", "Boolean", data_type="boolean", abstract="Make your choice :)", default="1"),
            LiteralInput("angle", "Angle", data_type="angle", abstract="Enter an angle [0, 360] :)", default="90"),
            LiteralInput("time", "Time", data_type="time", abstract="Enter a time like 12:00:00", default="12:00:00"),
            LiteralInput("date", "Date", data_type="date", abstract="Enter a date like 2012-05-01", default="2012-05-01"),
            LiteralInput(
                "datetime", "Datetime", data_type="dateTime", abstract="Enter a datetime like 2016-09-02T12:00:00Z", default="2016-09-02T12:00:00Z"
            ),
            LiteralInput(
                "string_choice",
                "String Choice",
            ...
            LiteralInput(
                "string_multiple_choice",
                "String Multiple Choice",
                abstract="Choose one or two items from list.",
                data_type="string",
                allowed_values=["sitting duck", "flying goose", "happy pinguin", "gentle albatros"],
                min_occurs=0,
                max_occurs=2,
            ...
            LiteralInput(
                ...
                metadata=[
                    Metadata("AllowedValue PyWPS Docs", "https://pywps.readthedocs.io/en/master/api.html#pywps.inout.literaltypes.AllowedValue"),
                    Metadata("AllowedValue Example", "http://docs.opengeospatial.org/is/14-065/14-065.html#98"),
                ],
                data_type="integer",
                default="1",
                allowed_values=[AllowedValue(minval=1, maxval=10), AllowedValue(minval=100, maxval=200, spacing=10)],
                mode=MODE.SIMPLE,
            ),
            LiteralInput(
                "any_value",
                ...
                metadata=[
                    Metadata("AnyValue PyWPS Docs", "https://pywps.readthedocs.io/en/master/api.html#pywps.inout.literaltypes.AnyValue"),
                ],
            ...
            LiteralInput(
                ...
                metadata=[
                    Metadata("PyWPS Docs", "https://pywps.readthedocs.io/en/master/_modules/pywps/inout/literaltypes.html"),
                ],
                data_type="string",
                allowed_values=ValuesReference(reference="https://en.wikipedia.org/w/api.php?action=opensearch&search=scotland&format=json"),
                default="Scotland",
                mode=MODE.SIMPLE,
            ),
        ...
        outputs = [
            ...
            LiteralOutput("string_multiple_choice", "String Multiple Choice", data_type="string"),
            ...
            ComplexOutput(
                "dataset", "Dataset", abstract="Copy of input netcdf file.", as_reference=True, supported_formats=[FORMATS.NETCDF, FORMATS.TEXT]
            ),
    ...
    def _handler(request, response):
        ...
        if "string_multiple_choice" in request.inputs:
            response.outputs["string_multiple_choice"].data = ", ".join([inpt.data for inpt in request.inputs["string_multiple_choice"]])
        else:
            response.outputs["string_multiple_choice"].data = "no value"
        response.outputs["int_range"].data = request.inputs["int_range"][0].data
        response.outputs["any_value"].data = request.inputs["any_value"][0].data
        response.outputs["ref_value"].data = request.inputs["ref_value"][0].data
